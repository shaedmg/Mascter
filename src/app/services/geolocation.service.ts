...
export class GeolocationService {
  ...
  constructor( 
    ...
    async startTrackingLocation(){
      ...
      this.tracking = true;
      const permissions = await Geolocation.requestPermissions().catch(err => null);
      if (permissions == null){
        await this.petProvider.updatePetLocation(this.petId, {ubication: null}).catch(_ => {});
    }else{
        this.watch = Geolocation.watchPosition({}, async (position, err) => {
        if ( position ) {
          this.petProvider.updatePetLocation(this.petId, {
            ubication: 
            {
          ...
          await this.stopTrackingLocation();
            if (!this.advertised){
              this.utilsService.showToast("Ubicacion no disponible, revise" +
  ...
  async stopTrackingLocation(){
    this.advertised = false;
      await Geolocation.clearWatch({ id: this.watch }).then(async () => {
        await this.petProvider.updatePetLocation(this.petId, {ubication: null}).catch(_ => {
